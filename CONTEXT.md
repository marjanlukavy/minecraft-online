...
рух одне одного. Деталі — розділ 12.

🏠 **РЕЖИМ «БУДИНОК-ДОСЛІДНИК» (поточний)** — гру перероблено: копання, руду, крафт і
мобів **вимкнено**. Світ = рівний газон-хаб + стилізований триповерховий будинок
(підвал/1-й/2-й) зі сходами-драбинами; стіни-фасад **стають прозорими, коли заходиш
усередину** (cutaway). Деталі — розділ 13.
...
## 13. Режим «Будинок-дослідник»

Гра більше НЕ про копання. Це затишний explorer: спавн на газоні-хабі, заходиш у
будинок, гуляєш поверхами по сходах. Мультиплеєр лишився (бачиш інших гравців).

- **Світ (детермінований, однаковий на клієнті й сервері):** `generateWorld()` у
  `js/world.js` та `server.js` — рівний газон (`GROUND=56`), декоративні дерева,
  будинок `buildHouse()`. Координати в об'єкті `HOUSE {x0:114,x1:133, roofTop:44,
  floorUp:50, floorGnd:56, floorBas:62}` мають збігатися у world.js, server.js, house.js.
- **Поверхи:** підвал (рядки 57–61), 1-й (51–55), 2-й (45–49); плити 50/56, підлога підвалу 62.
- **Сходи** = блок `B.STAIRS=17` (`solid:false, climb:true`). Шахти: вгору x=131 (рядки 48–55),
  вниз x=116 (54–61), проходять крізь плити. Лазіння — `overlapsClimb()` (world.js) +
  гілка в `updatePlayer` (player.js): на сходах гравітація off, W/↑ вгору, S/↓ вниз
  (`CLIMB_SPEED`). Двері — отвір у лівій стіні (x=114, рядки 54,55).
- **`js/house.js`** (підключений після render.js): `houseFacade` (alpha 1→0.1),
  `updateHouse()` (виклик у main.js), `playerInsideHouse()`; `drawHouseInterior()`
  (шпалери кімнат + меблі + драбини, рендериться ПЕРЕД гравцями), `drawHouseFacade()`
  (стилізований котедж: фахверк, двосхилий дах, димар, вікна, двері, вивіска — ПІСЛЯ
  гравців, зникає всередині), `drawHouseLights()` (тепле світло вікон у composite "lighter").
- **Вимкнено:** `updateMobs`/`drawMobs` не викликаються (mobs.js лишився, але мертвий);
  `handleMouseAction`/копання/крафт не викликаються; хотбар прихований (CSS), стартових
  предметів немає; інвентар/слоти/колесо прибрано з input.js. day/night лишився для атмосфери.
- **Тести:** структура світу — `scratchpad/wsworld.js` (через ws); headless-клієнт
  (рендер/фасад/лазіння) — `scratchpad/probe.js` (vm + заглушки DOM).
